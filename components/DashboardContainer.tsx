...
export default function DashboardContainer({ children }: DashboardContainerProps) {
  ...
  return (
    <div className="w-screen h-screen bg-gradient-to-br from-[#0d1c32] via-[#1a2f4a] to-[#0d1c32] flex flex-col items-center justify-center px-4 py-2">
      <SecurityGuard />
      {/* Modern Container */}
      <div className="w-full bg-white dark:bg-slate-900 rounded-[24px] border-[2px] border-black/30 dark:border-white/10 overflow-hidden flex h-[92vh] shadow-2xl shadow-black/30 hover:shadow-2xl hover:shadow-cyan-500/10">
        {/* LEFT SIDEBAR */}
        <div className={`${sidebarExpanded ? 'w-48' : 'w-20'} bg-white dark:bg-slate-950 border-r-[2px] border-black/20 dark:border-white/10 flex flex-col items-center py-6 transition-[width] duration-300 ease-out will-change-[width]`}>
          {/* Logo/ALTUS */}
          ...
          <div className="w-full flex flex-col gap-2.5 px-2 mb-3">
            ...
            <Link href="/dashboard/settings" className="flex items-center justify-center w-full p-3 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-transparent hover:bg-gradient-to-r hover:from-cyan-400/10 hover:to-emerald-400/10 border-[2px] border-transparent hover:border-cyan-400/30 rounded-[12px] transition-all duration-300 hover:shadow-md hover:shadow-cyan-400/20 active:scale-95 group">
            ...
            <Link href="/dashboard/settings" className={`flex cursor-pointer group gap-3 items-center transition-all duration-300 ${sidebarExpanded ? 'flex-row px-3 py-1' : 'flex-col justify-center px-2 py-1'}`}>
              ...
              {sidebarExpanded && (
                <div className="text-left min-w-0">
                  <p className="text-slate-900 dark:text-slate-100 font-bold text-xs leading-tight truncate">{user?.name || 'User'}</p>
                  <p className="text-slate-500 dark:text-slate-400 text-[10px] font-medium capitalize">{user?.role}</p>
                </div>
              )}
            </Link>
          </div>
